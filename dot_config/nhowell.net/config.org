...
if [[ $(hostname) == "nathan-edl" ]]; then
    # work
    GTK_THEME_LIGHT='Pop'
    GTK_THEME_DARK='Pop-dark'
else
    # home
...
go_light () {
    ...
    /usr/bin/gsettings set org.gnome.desktop.interface color-scheme "default"
    /usr/bin/gsettings set org.gnome.desktop.interface gtk-theme $GTK_THEME_LIGHT
}

go_dark () {
    echo "Currently not dark, going dark"
    /usr/bin/gsettings set org.gnome.desktop.interface color-scheme "prefer-dark"
    /usr/bin/gsettings set org.gnome.desktop.interface gtk-theme $GTK_THEME_DARK
}

if [[ "$SET_TO" == "light" ]]; then
    go_light
elif [[ "$SET_TO" == "dark" ]]; then
    go_dark
elif [[ "$current_scheme" == "prefer-dark" ]]; then
    go_light
elif [[ "$current_scheme" == "default" ]]; then
    go_dark
fi
#+end_src
...
* Sway
:PROPERTIES:
:header-args: :tangle (if (executable-find "sway") "~/.config/sway/config" "no") :mkdirp yes
:END:
...
*** Aliases
With a normal nvm install (which doesn't support fish), install bass to wrap the bash support to make everything work in fish. It's all nice and transparent once this alias is in place.

Install bass with =fisher add edc/bass= ([[https://github.com/edc/bass][GitHub - edc/bass: Make Bash utilities usable in Fish shell]]).

#+begin_src fish :tangle ~/.config/fish/functions/nvm.fish
function nvm
...
function firefox --description 'firefox flatpak'
    command flatpak run org.mozilla.firefox $argv
end
#+end_src

#+begin_src shell :tangle (if (executable-find "firefox") "no" "~/.local/bin/firefox") :shebang #!/bin/bash
...
Use the fish user key bindings function to load the default vi bindings and then override some of them.

#+begin_src fish :tangle ~/.config/fish/functions/fish_user_key_bindings.fish :mkdirp yes
function fish_user_key_bindings
    fish_vi_key_bindings

    bind --user -M default s 'fish_vi_run_count forward-char'
    bind --user -M default t 'fish_vi_run_count up-or-search'
    bind --user -M default n 'fish_vi_run_count down-or-search'

    bind --user -M operator s 'fish_vi_exec_motion forward-char'
    bind --user -M operator t 'fish_vi_exec_motion up-line'
    bind --user -M operator n 'fish_vi_exec_motion down-line'

    bind --user -M operator k --sets-mode k ''
    bind --user -M operator K --sets-mode K ''

    bind --user -M k '' get-key 'fish_vi_exec_motion commandline --forward-jump-till=$fish_key' 'set -eg fish_key'
    bind --user -M K '' get-key 'fish_vi_exec_motion commandline --backward-jump-till=$fish_key' 'set -eg fish_key'

    bind --erase --preset -m insert s
    bind --erase --preset -m insert S

    bind --user k forward-jump-till
    bind --user K backward-jump-till

    bind --user -M visual s forward-char
    bind --user -M visual t up-line
    bind --user -M visual n down-line
    bind --user -M visual k forward-jump-till
    bind --user -M visual K backward-jump-till
    bind --erase --preset -M visual -m insert s
end
#+end_src
...
function fish_prompt --description 'Write out the prompt'
    ...
    switch "$USER"
        ...
        case '*'
            set color_cwd $fish_color_cwd
            # the width of the rocket causes wrapping issues with long command lines in emacs/vterm
            # set suffix ' '
            set suffix '❯ '
    end
    ...
    echo -s ''
    echo -s (fish_mode_prompt2) $duration (set_color $fish_color_user) "$USER" $normal @ (set_color $fish_color_host) (prompt_hostname) $normal ' ' (set_color $color_cwd) (prompt_pwd) $normal (__fish_vcs_prompt) $normal $k8senv
...
Renaming this so that I can place the mode indicator in the main prompt myself.

#+begin_src fish
function fish_mode_prompt2 --description "Display my mode indicator for the prompt"
...
Set the default mode indicator to do nothing to get it out of the way.
