...
- Would like a way to have tmux next/prev-window commands switch between only unattached windows
...
active_windows=($(for sess in $(tmux list-sessions -F '#S' | grep '^main-.*'); do tmux list-windows -t $sess -F '#{?window_active,#I,}'; done))
unique_active_windows=($(printf "%s\n" "${active_windows[@]}" | sort -u))
unique_inactive_windows=($(echo ${windows[@]} ${unique_active_windows[@]} | tr ' ' '\n' | sort | uniq -u))

echo ${unique_inactive_windows[@]}
